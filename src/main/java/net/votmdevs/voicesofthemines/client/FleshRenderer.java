...
public class FleshRenderer extends GeoEntityRenderer<FleshEntity> {
    private static final ResourceLocation SELECT_ICON = new ResourceLocation(KerfurMod.MODID, "textures/gui/select.png");

    public FleshRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new FleshModel());
    }

    @Override
    public void render(FleshEntity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        this.shadowRadius = 0.2f + (entity.getFleshLevel() * 0.05f);
        super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);

        Minecraft mc = Minecraft.getInstance();

        if (mc.crosshairPickEntity == entity && entity.getFleshLevel() < 5 && !entity.isHeld()) {
            ...
            if (mc.player != null && mc.player.distanceTo(entity) < 5.0D) {
                ...
                float scale = 1.0f + (entity.getFleshLevel() * 0.15f);
