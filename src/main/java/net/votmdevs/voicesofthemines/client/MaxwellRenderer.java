...
public class MaxwellRenderer extends GeoEntityRenderer<MaxwellEntity> {
    private static final ResourceLocation SELECT_ICON = new ResourceLocation(VoicesOfTheMines.MODID, "textures/gui/select.png");

    public MaxwellRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new MaxwellModel());
        this.shadowRadius = 0.4f;
    }

    @Override
    public void render(MaxwellEntity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);

        Minecraft mc = Minecraft.getInstance();

        if (mc.crosshairPickEntity == entity && !entity.isHeld() && !entity.isSkeleton()) {
