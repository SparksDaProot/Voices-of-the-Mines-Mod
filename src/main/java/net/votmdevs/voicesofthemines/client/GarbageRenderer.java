...
public class GarbageRenderer extends GeoEntityRenderer<GarbageEntity> {
    private static final ResourceLocation SELECT_ICON = new ResourceLocation(KerfurMod.MODID, "textures/gui/select.png");

    public GarbageRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new GarbageModel());
    }

    @Override
    public RenderType getRenderType(GarbageEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(texture);
    }

    @Override
    public void render(GarbageEntity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        this.shadowRadius = 0.2f + (entity.getGarbageLevel() * 0.05f);
        super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);

        Minecraft mc = Minecraft.getInstance();

        if (mc.crosshairPickEntity == entity && entity.getGarbageLevel() < 5 && !entity.isHeld()) {
            ...
            if (mc.player != null && mc.player.distanceTo(entity) < 5.0D) {
                ...
                float scale = 1.0f + (entity.getGarbageLevel() * 0.15f);
